...
export default function Trade() {
    ...
    const handleChange = (e) => {
        ...
        if (name === 'offeredCard' || name === 'requestedCard') {
            const normalizedCardValue = value.replace('🃏', '').trim().split('|')[0].trim();
            const userData = name === 'offeredCard' ? currentUser.cards : selectedUser.cards;
            const cardFound = userData.find(card => {
                const normalizedCardName = card.card_name.replace('🃏', '').trim().split('|')[0].trim();
    ...
    useEffect(() => {
        const fetchAllUsers = async () => {
            ...
            setUserAllData(data);
            setCurrentUser(data.users.find(user => user.user_id === session.user.databaseId));
            setSelectedUser(data.users.find(user => user.user_id === selectedUserId));
        }
        ...
        <div className="min-h-screen bg-gradient-to-br from-blue-100 to-purple-100 py-16 flex flex-col items-center justify-center">
            ...
            <div className="container mx-auto px-4 sm:px-6 lg:px-8 w-full max-w-3xl">
                <header className="text-center mb-8">
                    <h1 className="text-3xl font-bold text-indigo-700">Obchody s kartičkami</h1>
                </header>
                        ...
                        <form onSubmit={createNewOffer}>
                            <div className="fixed top-0 left-0 w-full h-full bg-gray-800 bg-opacity-75 flex justify-center items-center">
                                <div className="bg-white p-8 rounded-md shadow-lg w-full max-w-lg">
                                    <h2 className="text-xl font-bold text-indigo-700 mb-4">Nová nabídka</h2>

                                    <div className="mb-4">
                                        <label className="block text-gray-700 font-semibold mb-2">Vyber svou kartičku:</label>
                                        <select 
                                        ...
                                        >
                                            {currentUser?.cards.map((card) => (
                                                <option 
                                                    ...
                                                    value={card.card_id}
                                                >
                                                    🃏 {card.card_name} | ({card.collection_name}) ({card.card_quantity})x
                                                </option>
                                            ))}
                                        </select>
                                    </div>
                                    ...
                                    <div className="mb-4 relative">
                                        <label className="block text-gray-700 font-semibold mb-2">Vyber uživatele:</label>
                                        <input
                                            type="text"
                                            name="toUserId"
                                            value={searchText}
                                        ...
                                        {selectedUser && (
                                            <p className="mt-2 text-sm text-gray-600">
                                                Vybraný uživatel: {selectedUser.user_name} (ID: {selectedUser.user_id})
                                            </p>
                                        )}
                                    </div>

                                    <div className="mb-4">
                                        <label className="block text-gray-700 font-semibold mb-2">Vyber kartičku od uživatele:</label>
                                        <select 
                                            className="w-full border rounded px-4 py-2"
                                            name="requestedCard"
                ...
                <section className="bg-white shadow-md rounded-lg p-6 mb-8">
                    <h2 className="text-xl font-semibold text-indigo-700 mb-4">Moje odeslané nabídky</h2>
                    {!isCreatingNewOffer && (
                        <>
                            {outgoingOffers.length === 0 ? (
                                ...
                                <ul className="divide-y divide-gray-200 overflow-y-auto max-h-60">
                                    ...
                                    {outgoingOffers.map((offer) => (
                                        <li key={offer.id} className="px-4 py-3 bg-white hover:bg-gray-100 grid grid-cols-4 gap-4 items-center overflow-hidden">
                                            <span className="flex items-center gap-2 relative w-40 h-40 flex-shrink-0">
                                                ...
                                                <div className="absolute left-0 bottom-0 text-center py-3 w-27">
                                                    <div className="text-xs font-semibold truncate text-white">{offer.offered_card.name}</div>
                                                    <div className="text-[0.5rem] text-gray-300 truncate">Kol: {offer.offered_card.collection_name}</div>
                                                    {offer.offered_card.rarity_name && (
                                                        <div className="text-[0.5rem] text-yellow-400 truncate">R: {offer.offered_card.rarity_name}</div>
                                                    )}
                                                </div>
                                            </span>
                                            <span className="flex items-center gap-2 relative w-40 h-40 flex-shrink-0">
                                                ...
                                                /> 
                                                <div className="absolute left-0 bottom-0 text-center py-3 w-27">
                                                    <div className="text-xs font-semibold truncate text-white">{offer.wanted_card.name}</div>
                                                    <div className="text-[0.5rem] text-gray-300 truncate">Kol: {offer.wanted_card.collection_name}</div>
                                                    {offer.wanted_card.rarity_name && (
                                                        <div className="text-[0.5rem] text-yellow-400 truncate">R: {offer.wanted_card.rarity_name}</div>
                                                    )}
                                                </div>
                                            </span>
                                            <span>
                ...
                <section className="bg-white shadow-md rounded-lg p-6">
                    <h2 className="text-xl font-semibold text-indigo-700 mb-4">Moje přijaté nabídky</h2>
                    {!isCreatingNewOffer && (
                        <>
                            {incomingOffers.length === 0 ? (
                                ...
                                <ul className="divide-y divide-gray-200">
                                    ...
                                    {incomingOffers.map((offer) => (
                                        <li key={offer.id} className="px-4 py-3 bg-white hover:bg-gray-100 grid grid-cols-4 gap-4 items-center overflow-hidden">
                                            <span className="text-xs font-semibold truncate text-gray-700">{offer.from_username}</span>
                                            <span className="flex items-center gap-2 relative w-40 h-40 flex-shrink-0">
                                                <img
                                                    src={offer.offered_card.image_path}
                                                    alt={`${offer.offered_card.name} Image`}
