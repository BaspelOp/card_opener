...
export default function Stats() {
      ...
      try {
        ...
        const responses = await Promise.all([
          fetch("/api/stats", {
            ...
            body: JSON.stringify({ statistic: "top-users" })
          }),
          fetch("/api/stats", {
            method: "POST",
            headers: { "Content-Type": "application/json" },
            body: JSON.stringify({ statistic: "most-owned" })
          }),
          fetch("/api/stats", {
            method: "POST",
            headers: { "Content-Type": "application/json" },
            body: JSON.stringify({ statistic: "most-wanted" })
          })
        ]);
        ...
        const [topUsersData, mostOwnedData, mostWantedData] = await Promise.all(
  ...
  if (error) {
    return (
      <div className="container mx-auto p-6 text-red-500 flex justify-center items-center">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
          <div className="bg-white shadow-md rounded-lg p-6 flex flex-col justify-center">
            <h2 className="text-xl font-semibold text-indigo-700 mb-4 text-center">
              Top 10 uživatelů s nejvíce kartami
            </h2>
          ...
          <div className="bg-white shadow-md rounded-lg p-6 flex flex-col justify-center">
            <h2 className="text-xl font-semibold text-indigo-700 mb-4 text-center">
              Top 10 nejžádanějších karet (ve směnárně)
            </h2>
            <div className="w-full h-60">
              <ResponsiveContainer>
